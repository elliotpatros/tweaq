...
{
    char* ext_pathout = change_extension(pathout, format);
    if (ext_pathout == 0) return 0;
    char* unique_pathout = unique_path(ext_pathout);
    free(ext_pathout);
...
extern "C"
{
    ...
    void* convert_handleInput(int argc, const char* argv[])
    {
        ...
        // get bit depth
        if      (strcmp(argv[kBitDepth], "32-bit float") == 0) input->bitDepth = SF_FORMAT_FLOAT;
        else if (strcmp(argv[kBitDepth], "32-bit PCM") == 0)   input->bitDepth = SF_FORMAT_PCM_32;
        else if (strcmp(argv[kBitDepth], "16-bit PCM") == 0)   input->bitDepth = SF_FORMAT_PCM_16;
        else if (strcmp(argv[kBitDepth], "8-bit PCM") == 0)    input->bitDepth = SF_FORMAT_PCM_S8;
        else                                                   input->bitDepth = SF_FORMAT_PCM_24; // default choice (24 bit)
        
        // get file format
        if      (strcmp(argv[kFormat], "aiff") == 0) input->format = SF_FORMAT_AIFF;
        else if (strcmp(argv[kFormat], "flac") == 0) input->format = SF_FORMAT_FLAC;
        else                                         input->format = SF_FORMAT_WAV; // default choice (wav)
    ...
    bool convert_process(const char* pathin, const char* pathout, void* args)
    {
        ...
        char* unique_pathout = get_pathout(pathout, input->format);
